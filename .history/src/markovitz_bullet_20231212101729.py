...
class MarkovitzSimulator:
    ...
    def _cov_rescaler(self, returns : np.ndarray, replace_value : np.ndarray) -> tuple[np.ndarray]:
        ...
        n = len(returns)
        rescale_matrix = n / n_mask
        
        return returns_imputed, rescale_matrix
    ...
    def make_random_portfolios(self, n_simulations: int = 1) -> "MarkovitzSimulator":
        ...
        returns, rescale_matrix = self._cov_rescaler(self.returns, mean_returns)
        ...
        covariance_matrix = np.cov(returns, rowvar = False, bias = True) * rescale_matrix
